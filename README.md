...
- **Category-Wise Personas System (30 Personas across 11 Specialized Categories)**: Backend swapper (`chat_swapper.py`) loads `config/personas.json` into memory on startup and exposes `GET /v1/personas` on port 8080, structured across 11 domain categories with concrete, actionable `prompt_blueprint` skillsets.
- **Dynamic Persona Selection Dropdown with Optgroups & Slash Commands**: Select personas directly from the categorized chat input footer dropdown or type slash shortcuts (`/prof`, `/algo`, `/cp`, `/theorist`, `/dl`, `/mlops`, `/safety`, `/nlp`, `/architect`, `/dev`, `/web`, `/db`, `/hack`, `/devops`, `/systems`, `/sre`, `/sec`, `/cloudsec`, `/paper`, `/review`, `/data`, `/flutter-android`, `/flutter-web`, `/flutter-desktop`, `/go-cloud`, `/static`, `/agent`, `/codereview`, `/terraform`, `/rust`). System prompts, specialized domain skills, and recommended target models (`podllama-thinking` / `podllama-chat` / `podllama-instruct`) are injected automatically into completions.
- **KaTeX LaTeX Math Rendering**: Renders inline (`$...$`) and display (`$$...$$`) mathematical equations, step-by-step proof derivations, and LaTeX algorithm notation directly within the chat panel.
...
│   ├── personas.json            # AI persona registry (prompt blueprints, LLM configs, slash commands)
...
### 3. Instruct Completions API (`podllama-instruct`)

```bash
curl http://localhost:4000/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer sk-local" \
  -d '{"model": "podllama-instruct", "messages": [{"role": "user", "content": "Refactor this Python function to use async/await."}]}'
```

### 4. Autocomplete Completion API (`podllama-autocomplete`)
...
### 5. Query Available Models API (`GET /v1/models`)
...
### 5. Personas API (`GET /v1/personas`)

Query the full persona registry from the chat supervisor, including all `prompt_blueprint`, `llm_config`, and `ui_metadata` for all 30 configured personas:
...
```json
{
  "global_config": { "base_system_directives": ["..."] },
  "categories": [{ "id": "cs-theory", "display_name": "Computer Science & Foundations", "..." }],
  "personas": [
    {
      "id": "cp-solver",
      "ui_metadata": { "slash_command": "/cp", "display_name": "Competitive Programming Solver" },
      "llm_config": { "target_model": "podllama-chat", "temperature": 0.1 },
      "prompt_blueprint": { "role_definition": "...", "strict_constraints": ["..."] }
    }
  ]
}
```
...
> [!NOTE]
> The `/v1/personas` endpoint is served directly by `chat_swapper.py` on port `8080`, not through the LiteLLM proxy on port `4000`. The VS Code extension fetches this on startup to populate the persona selection dropdown.

### 6. Model Aliases & Registry Map
